...
export const StaggeredMenu = ({
  ...
  useEffect(() => {
    if (open) {
      gsap.to('.hero-shuffle', {
        scale: 0.55,
        x: position === 'right' ? '-22vw' : '22vw',
        duration: 0.8,
        ease: 'power4.inOut'
      });
    } else {
  ...
  const buildOpenTimeline = useCallback(() => {
    ...
    layerStates.forEach((ls: any, i: number) => {
      tl.fromTo(ls.el, { xPercent: ls.start }, { xPercent: 0, duration: 0.5, ease: 'power4.inOut' }, i * 0.07);
    });
  ...
  return (
    ...
    >
      ...
      <header className="staggered-menu-header pt-8 pb-12 pointer-events-none" aria-label="Main navigation header">
        <div className="sm-logo pointer-events-auto" aria-label="Logo">
          <a href="/" className="z-50 mix-blend-difference" style={{ ...(open ? { mixBlendMode: 'normal' } : {}), display: 'inline-block', height: '40px' }}>
            <TextPressure
              text="INVEGA CAPITAL"
              flex={false}
              alpha={false}
        ...
        <button
          ...
          type="button"
        >
          <span ref={textWrapRef} className="sm-toggle-textWrap text-xs uppercase tracking-widest font-bold" aria-hidden="true">
