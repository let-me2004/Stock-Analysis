...
export async function getFullAnalysis(ticker) {
  ...
  const keyMetrics = incomeMap.map((inc, i) => {
    ...
    return {
      date: inc.date,
      revenuePerShare: inc.revenue ? inc.revenue / shares : null,
      netIncomePerShare: inc.netIncome ? inc.netIncome / shares : null,
      operatingCashFlowPerShare: cf.operatingCashFlow ? cf.operatingCashFlow / shares : null,
      freeCashFlowPerShare: cf.freeCashFlow ? cf.freeCashFlow / shares : null,
      bookValuePerShare: bs.totalEquity ? bs.totalEquity / shares : null,
      roic: inc.operatingIncome && (bs.totalEquity && bs.totalDebt) ? inc.operatingIncome / (bs.totalEquity + bs.totalDebt) : null,
      roe: inc.netIncome && bs.totalEquity ? inc.netIncome / bs.totalEquity : null,
      returnOnCapitalEmployed: inc.operatingIncome && bs.totalAssets && bs.currentLiabilities ? inc.operatingIncome / (bs.totalAssets - bs.currentLiabilities) : null,
      currentRatio: bs.currentAssets && bs.currentLiabilities ? bs.currentAssets / bs.currentLiabilities : null,
      debtToEquityRatio: bs.totalDebt && bs.totalEquity ? bs.totalDebt / bs.totalEquity : null,
      grossProfitMargin: inc.grossProfit && inc.revenue ? inc.grossProfit / inc.revenue : null,
      netProfitMargin: inc.netIncome && inc.revenue ? inc.netIncome / inc.revenue : null,
      operatingProfitMargin: inc.operatingIncome && inc.revenue ? inc.operatingIncome / inc.revenue : null,
      freeCashFlowMargin: cf.freeCashFlow && inc.revenue ? cf.freeCashFlow / inc.revenue : null,
    }
  })
  ...
  const ratios = incomeMap.map((inc, i) => {
    ...
    return {
      ...
      netProfitMargin: inc.netIncome && inc.revenue ? inc.netIncome / inc.revenue : null,
      operatingProfitMargin: inc.operatingIncome && inc.revenue ? inc.operatingIncome / inc.revenue : null,
      returnOnEquity: inc.netIncome && bs.totalEquity ? inc.netIncome / bs.totalEquity : null,
      returnOnAssets: inc.netIncome && bs.totalAssets ? inc.netIncome / bs.totalAssets : null,
      debtToEquityRatio: bs.totalDebt && bs.totalEquity ? bs.totalDebt / bs.totalEquity : null,
      currentRatio: bs.currentAssets && bs.currentLiabilities ? bs.currentAssets / bs.currentLiabilities : null,
      dividendPayoutRatio: cf.dividendsPaid && inc.netIncome ? Math.abs(cf.dividendsPaid) / inc.netIncome : null,
      capexToRevenue: cf.capitalExpenditure && inc.revenue ? Math.abs(cf.capitalExpenditure) / inc.revenue : null,
      capexToOperatingCashFlow: cf.capitalExpenditure && cf.operatingCashFlow ? Math.abs(cf.capitalExpenditure) / cf.operatingCashFlow : null,
      freeCashFlowToRevenue: cf.freeCashFlow && inc.revenue ? cf.freeCashFlow / inc.revenue : null,
    }
  })
  ...
  const keyMetricsTTM = [{
    ...
    freeCashFlowMargin: fd.freeCashflow && fd.totalRevenue ? fd.freeCashflow / fd.totalRevenue : null,
    freeCashFlowMarginTTM: fd.freeCashflow && fd.totalRevenue ? fd.freeCashflow / fd.totalRevenue : null,
